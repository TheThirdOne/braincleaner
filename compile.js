...
function checkExprs(name, body, state){
  for(var i = 0; i < block.length;i++){
    if(block[i].type === 'while'){
    ...
    if(block[i].type === 'statement'){ // make sure a main b += ... doesn't happen
      for(var k = 0; k < block[i].assignment.length; k++){
        if(block[i].assignment[k].type !== 'identifier' || state.fcns[name].arg[block[i].assignment[k].data] === undefined){
          throw "In Function " + name + ": Only variables allowed in assignment \"" + block[i].assignment.join(" ") + "\"";
        }
      }
    }
    if(block[i].type === 'statement' && !!state.fcns[block[i].expression[0]]){ //for function calls
      var fcn = state.fcns[block[i].expression[0]];
      ...
      for(var k = 0; k < block[i].expression.length; k++){ //check for ... main a b + h;
        if(block[i].expression[k].type !== 'identifier' || state.fcns[name].arg[block[i].expression[k].data] === undefined){
      ...
      for(var k = 0; k < block[i].expression.length; k++){ //check for ... main a b + h;
        if(block[i].expression[k].type !== 'identifier' || state.fcns[name].arg[block[i].expression[k].data] === undefined){
          stack++;
        }else if(block[i].expression[k].type !== '+' || block[i].expression[k].type !== '-' || block[i].expression[k].type !== '*' || block[i].expression[k].type !== '%'){
          stack--;
        }
      }
      if(stack !== 1){
